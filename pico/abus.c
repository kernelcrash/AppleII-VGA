...
// For MSX/TMS9918a the PHI0 line is the _IORQ. IO addresses 0x98 and 0x99 are used by the TMS9918A
//   This code assumes you have direct access to the active low IO chip select for 0x98/0x99. For most
//   implementations this won't be possible. I'd suggest looking at this same section of abus.c in the
//   v9938 branch to see how you can adapt this to read the full address, and determine if its an access
//   to 0x98/0x99. That code in the v9938 branch is specifically for an MSX2 computer, so it would 
//   require very little change to work with MSX1
void abus_loop() {
    while(1) {
