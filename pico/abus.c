...
// Shadow parts of the Coco/Dragon memory by observing the bus write cycles
static void shadow_memory(bool is_write, uint_fast16_t address, uint32_t value) {

    // only consider writes
    if (is_write) {
        switch (address >> 5) {
            // SAM registers at FFC0 to FFDF
            case (0xffc0 >> 5):
                mc6847_sam_write(address & 0x1f);
                break;
            default:
                if (address == 0xff22) {
                    mc6847_set_pia_mode(value>>3);
                }
...
void abus_loop() {
    while(1) {
        ...
        //if(is_devsel) {
	// devsel is ignored so you dont need to wire it up
