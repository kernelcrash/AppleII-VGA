...
#if CONFIG_PIN_Z80_WR != _WR_GPIO
#error CONFIG_PIN_Z80_WR and _WR_GPIO must be set to the same pin
#endif
...
static void abus_main_setup(PIO pio, uint sm) {
    ...
    //sm_config_set_jmp_pin(&c, CONFIG_PIN_Z80_IORQ);

    // map the IN pin group to the data signals
    sm_config_set_in_pins(&c, CONFIG_PIN_Z80_DATA_BASE);
    ...
    pio_sm_set_pindirs_with_mask(pio, sm, (0x7 << CONFIG_PIN_APPLEBUS_CONTROL_BASE),
        (1 << CONFIG_PIN_Z80_WR) | (0x7 << CONFIG_PIN_APPLEBUS_CONTROL_BASE) | (0x3ff << CONFIG_PIN_Z80_DATA_BASE));
    ...
    pio->input_sync_bypass |= (0x3ff << CONFIG_PIN_Z80_DATA_BASE);

    pio_gpio_init(pio, CONFIG_PIN_Z80_WR);
    gpio_set_pulls(CONFIG_PIN_Z80_WR, false, false);

    for(int pin = CONFIG_PIN_APPLEBUS_CONTROL_BASE; pin < CONFIG_PIN_APPLEBUS_CONTROL_BASE + 3; pin++) {
        pio_gpio_init(pio, pin);
    }

    for(int pin = CONFIG_PIN_Z80_DATA_BASE; pin < CONFIG_PIN_Z80_DATA_BASE + 10; pin++) {
        pio_gpio_init(pio, pin);
...
void abus_loop() {
    while(1) {
        ...
        const bool is_mreq = ((value & (1u << (CONFIG_PIN_Z80_MREQ - CONFIG_PIN_Z80_DATA_BASE))) == 0);
        const bool is_iorq = ((value & (1u << (CONFIG_PIN_Z80_IORQ - CONFIG_PIN_Z80_DATA_BASE))) == 0);
        if(is_mreq) {
            // Not a refresh cycle
            uint_fast16_t address = (value >> 10) & 0xffff;
            shadow_memory(1, address, value);
	}
	if (is_iorq) {
            uint_fast16_t address = (value >> 10) & 0x0001;
            if (!address) {
